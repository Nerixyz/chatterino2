...
void TwitchIrcServer::initializeConnection(IrcConnection *connection,
                                           ConnectionType type)
{
    bool useAnon = type == ConnectionType::Read &&
                   getSettings()->twitchReadConnectionMode !=
                       TwitchReadConnectionMode::Authenticated;
    auto init = [connection = QPointer(connection), type,
                 useAnon](const std::shared_ptr<TwitchAccount> &account) {
        if (!connection)
        {
            return;
        }
    ...
    if (useAnon)
    {
        init(getApp()->getAccounts()->twitch.getCurrent());
        return;
    }

    getApp()->getAccounts()->twitch.requestCurrentChecked(
        [init](const std::shared_ptr<TwitchAccount> &account) {
            init(account);
        });
}
