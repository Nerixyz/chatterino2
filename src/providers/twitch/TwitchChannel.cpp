...
{
    auto added = BttvEmotes::addEmote(this->getDisplayName(), this->bttvEmotes_,
                                      message);
    this->addOrReplaceLiveUpdatesAddRemove(
        MessageBuilder(liveUpdatesAddEmoteMessage, "BTTV", QString(),
                       {added->name.string})
            .release(),
        "BTTV");
}
...
{
    if (auto removedName = BttvEmotes::removeEmote(this->bttvEmotes_, message))
    {
        this->addOrReplaceLiveUpdatesAddRemove(
            MessageBuilder(liveUpdatesRemoveEmoteMessage, "BTTV", QString(),
                           {*removedName})
                .release(),
            "BTTV");
    }
    ...
    const SeventvEventApiEmoteAddDispatch &dispatch)
{
    ...
    builder.message().flags.set(MessageFlag::LiveUpdatesPlatform);
    ...
    const SeventvEventApiEmoteRemoveDispatch &dispatch)
{
    ...
    auto builder = MessageBuilder(liveUpdatesRemoveEmoteMessage, "7TV",
            ...
            postToThread([this, weak, dispatch, emotes, name]() {
                if (auto shared = weak.lock())
                {
                    ...
                    builder.message().flags.set(
                        MessageFlag::LiveUpdatesPlatform);
                    this->addMessage(builder.release());
                }
            });
        },
...
void TwitchChannel::addOrReplaceLiveUpdatesAddRemove(MessagePtr message,
                                                     const QString &platform)
{
    ...
    bool platformFlag = message->flags.has(MessageFlag::LiveUpdatesPlatform);
    MessageFlag currentFlag = {};
    if (message->flags.has(MessageFlag::LiveUpdatesAdd))
    {
        currentFlag = MessageFlag::LiveUpdatesAdd;
    }
    else if (message->flags.has(MessageFlag::LiveUpdatesRemove))
    {
    ...
    for (int i = snapshotLength - 1; i >= end; --i)
    {
        ...
        if (s->flags.has(MessageFlag::LiveUpdatesPlatform) != platformFlag)
        {
            continue;
        }
...
void TwitchChannel::joinBttvChannel()
{
    if (getApp()->twitch->bttvLiveUpdates)
    {
