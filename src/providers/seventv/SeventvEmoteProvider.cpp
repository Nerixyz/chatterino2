    ...
    TwitchChannel *channel, const seventv::eventapi::EmoteAddDispatch &message)
{
    ...
    if (this->addChannelEmote(channel->channelEmotes(), emote))
    {
        return emote;
    }
    return nullptr;
    ...
    const seventv::eventapi::EmoteUpdateDispatch &message)
{
    return this->updateChannelEmote(
        channel->channelEmotes(), message.emoteID, message.oldEmoteName,
        [&](const auto &oldPtr) {
...
{
    return this->removeChannelEmote(channel->channelEmotes(), message.emoteID,
                                    message.emoteName);
}
    ...
    const std::function<void(ExpectedStr<EmoteSetData>)> &onDone)
{
    ...
    std::weak_ptr selfWP = std::static_pointer_cast<SeventvEmoteProvider>(
        this->shared_from_this());
    getApp()->getSeventvAPI()->getEmoteSet(
        emoteSetID,
        [emoteSetID, onDone, selfWP,
         chanWP = channel->weakTwitchChannel()](const auto &json) {
            assert(!isAppAboutToQuit());
            auto self = selfWP.lock();
            auto channel = chanWP.lock();
            if (!self || !channel)
            {
                return;
            }

            auto *data = channel->channelEmotes().dataForProvider(self);
            if (!data)
            {
                return;
            }
