    ...
    const std::shared_ptr<MessageThread> &thread, const MessagePtr &parent)
{
    ...
    MessageBuilder::processIgnorePhrases(
        *getSettings()->ignoredMessages.readOnly(), content, twitchEmotes);
                                       ...
                                       const EmoteName &name)
{
    ...
    //  - BetterTTV Global
    //  - 7TV Global
    if (twitchChannel && (emote = twitchChannel->ffzEmote(name)))
    {
        flags = MessageElementFlag::FfzEmote;
    }
    else if (twitchChannel && (emote = twitchChannel->bttvEmote(name)))
    {
        flags = MessageElementFlag::BttvEmote;
    }
    else if (twitchChannel != nullptr &&
                                        ...
                                        TwitchChannel *twitchChannel)
{
    ...
    auto badgeInfos = MessageBuilder::parseBadgeInfoTag(tags);
    auto badges = parseBadgeTag(tags);
