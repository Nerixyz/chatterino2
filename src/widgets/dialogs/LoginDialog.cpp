...
{
    connect(&this->pingTimer_, &QTimer::timeout, [this] {
        this->ping();
    });
    connect(&this->expiryTimer_, &QTimer::timeout, [this] {
    ...
    connect(startButton, &QPushButton::clicked, this, [this] {
        ...
        NetworkRequest(u"https://id.twitch.tv/oauth2/device"_s,
            ...
            .onError([this](const auto &res) {
                this->displayError(res.parseJson()["message"_L1].toString(
                    u"error: (no message)"_s));
            })
            .execute();
    });
...
void DeviceLoginWidget::tryInitSession(const QJsonObject &response)
{
    ...
    auto expiry = response["expires_in"_L1];
    if (!expiry.isDouble())
    {
    ...
    // </a>
    auto *userCode =
        new QLabel(u"<a href=\"%1\"><span>%2</span>"_s
                   "<span style=\"font-size: large\">%3</span>"
                   "<span>%4</span></a>".arg(this->verificationUri_, prefixUrl,
                                             this->userCode_, postfixUrl));
    ...
    {
        userCode->setText("You're in streamer mode.\nUse the buttons "
                          "below.\nDon't show the code on stream!");
    }
...
void DeviceLoginWidget::ping()
{
    ...
    NetworkRequest(u"https://id.twitch.tv/oauth2/token"_s,
        ...
        .payload(query.toString(QUrl::FullyEncoded).toUtf8())
        .onSuccess([this](const auto &res) -> Outcome {
            ...
            helix->fetchUsers(
                ...
                [self, helix, refreshToken, accessToken,
                 expiresAt](const auto &res) -> Outcome {
                    if (res.empty())
                    {
                        if (self)
                        {
