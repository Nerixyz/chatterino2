...
///
/// To add content in a derived class, implement #paintContent().
/// This gets called before the click and hover effects are drawn.
/// By default the button caches the content drawn in #paintContent(),
/// as the button often repaints due to mouse movement (and the hover effects).
/// When updating your button, you can invalidate the cache with #invalidateContent().
/// You can opt out of this by calling #setContentCacheEnabled().
...
public:
    ...
    ///
    /// An enabled button will emit interactions events and show mouse effects.
    ...
    ///
    /// @sa #menu(), #setMenu()
    [[nodiscard]] bool menuVisible() const
    {
    ...
    ///
    /// The border is shown with a width of 1 above all other content
    /// if the border color is valid.
    ///
