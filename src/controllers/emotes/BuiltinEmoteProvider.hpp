...
protected:
    ...
    bool addChannelEmote(ChannelEmotes &channelEmotes, EmotePtr emote);

    /// Update an emote by its ID (with an optional hint)
    ///
    /// @param channelEmotes The channel emotes
    ...
    ///          pointer.
    EmotePtr removeChannelEmote(ChannelEmotes &channelEmotes, const QString &id,
                                const QString &nameHint);
