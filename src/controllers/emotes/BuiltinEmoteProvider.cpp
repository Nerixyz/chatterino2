    ...
    LoadChannelArgs args)
{
    ...
    NetworkRequest(this->channelEmotesUrl(*twitch))
        .timeout(20000)
        .onSuccess([onDone, twitchWP = std::weak_ptr(twitch),
                    selfWP = this->weak_from_this()](const auto &result) {
            auto twitch = twitchWP.lock();
            auto self = selfWP.lock();
            if (!twitch || !self)
            {
                return;
            }

            auto emotes = self->parseChannelEmotes(*twitch, result.parseJson());
            if (emotes)
            {
    ...
    std::function<void(ExpectedStr<void>)> onDone)
{
    ...
    NetworkRequest(this->globalUrl)
        ...
        .onSuccess([onDone,
            ...
            writeProviderEmotesCache("global", self->id(), result.getData());
        ...
        .onError([onDone, selfWP = this->weak_from_this()](const auto &result) {
            auto self = selfWP.lock();
            if (!self)
            {
                return;
            ...
            qCWarning(chatterinoEmoji) << "Failed to fetch global BTTV emotes. "
                                       << result.formatError();

            readProviderEmotesCache("global", "betterttv", [&](const auto &jv) {
...
bool BuiltinEmoteProvider::supportsChannel(Channel *channel)
{
...
bool BuiltinEmoteProvider::addChannelEmote(ChannelEmotes &channelEmotes,
                                           EmotePtr emote)
{
    auto *data = channelEmotes.dataForProvider(this->weak_from_this());
    if (!data)
    {
        return false;
    }
...
std::optional<std::pair<EmotePtr, EmotePtr>>
    BuiltinEmoteProvider::updateChannelEmote(
        ChannelEmotes &channelEmotes, const QString &id,
...
EmotePtr BuiltinEmoteProvider::removeChannelEmote(ChannelEmotes &channelEmotes,
                                                  const QString &id,
                                                  const QString &nameHint)
{
    auto *item = channelEmotes.dataForProvider(this->weak_from_this());
    if (!item)
    {
        return nullptr;
    }
