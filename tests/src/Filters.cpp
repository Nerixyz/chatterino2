...
TEST(Filters, Validity)
{
    ...
    for (const auto &[input, expected] : tests)
    {
        ...
        bool isValid = filterResult.has_value();
        EXPECT_EQ(isValid, expected)
            << "Filter::fromString( " << input << " ) should be "
            << (expected ? "valid" : "invalid") << " error: "
            << (filterResult ? u"(none)"_s : filterResult.error());
    }
}
...
TEST(Filters, TypeSynthesis)
{
    ...
    for (const auto &[input, expected] : tests)
    {
        ...
        auto filter = *std::move(filterResult);
        T type = filter.returnType();
        EXPECT_EQ(type, expected) << "Filter{ " << input << " } has type "
