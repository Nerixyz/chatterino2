...
extern int tstBasicFullMatch(const char16_t *in, size_t len) noexcept;
extern std::pair<int, size_t> tstBasicPartialMatch(const char16_t *in,
                                                   size_t len) noexcept;
extern int tstUnicodeFullMatch(const char16_t *in, size_t len) noexcept;
extern std::pair<int, size_t> tstUnicodePartialMatch(const char16_t *in,
                                                     size_t len) noexcept;
...
void compareFixture(const char *name, std::string_view data, bool write = false)
{
    auto path =
...
auto printToStr(const std::unique_ptr<IntNode> &root,
                triegen::PrintOptions options)
{
    std::ostringstream buf;
    triegen::printTree(buf, root, options);
    return buf.str();
}
...
TEST(triegenTree, print)
{
    ...
    compareFixture("BasicFullMatch.fixture.cpp",
                   printToStr(root, {
                                        .fnName = "tstBasicFullMatch",
                                        .fullMatch = true,
                                    }));
    compareFixture("BasicPartialMatch.fixture.cpp",
                   printToStr(root, {
...
{
    auto m = [](std::u16string_view v) {
        return triegen::tstBasicFullMatch(v.data(), v.length());
    };
...
{
    auto m = [](std::u16string_view v, int expectedValue, size_t expectedLen) {
        auto [value, len] = triegen::tstBasicPartialMatch(v.data(), v.length());
        ASSERT_EQ(value, expectedValue) << triegen::toRegular(v);
        ASSERT_EQ(len, expectedLen) << triegen::toRegular(v);
    };
...
TEST(triegenTree, printUnicode)
{
    ...
    compareFixture("UnicodeFullMatch.fixture.cpp",
                   printToStr(root, {
                                        .fnName = "tstUnicodeFullMatch",
                                        .fullMatch = true,
                                    }));

    compareFixture("UnicodePartialMatch.fixture.cpp",
                   printToStr(root, {
...
{
    auto m = [](std::u16string_view v) {
        return triegen::tstUnicodeFullMatch(v.data(), v.length());
    };
...
TEST(triegenGen, unicodePartialMatch)
{
    auto m = [](std::u16string_view v, int expectedValue, size_t expectedLen) {
        auto [value, len] =
            triegen::tstUnicodePartialMatch(v.data(), v.length());
        ASSERT_EQ(value, expectedValue) << triegen::toRegular(v);
        ASSERT_EQ(len, expectedLen) << triegen::toRegular(v);
    };
