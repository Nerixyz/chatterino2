...
template <typename T>
size_t commonPrexixLength(std::basic_string_view<T> left,
...
void Node<T>::add(std::u16string_view view, T value, size_t totalLength)
{
    ...
    auto vis = detail::Overloaded{
        ...
        [&](Run<T> &node) {
            ...
            auto prefix = detail::commonPrexixLength(node.run, view);
...
{
    out << std::boolalpha << "#include <cstring>\n";
    ...
    {
        out << "std::pair<" << triegen::value::valueName<T>() << ", size_t> ";
    }
    out << options.fnName;
    out << "(const char16_t *in, size_t len) noexcept {\n";
